...
def _ctx(**kwargs):
    ...
    try:
        ...
        wn.synsets("dog")  # اطمینان از بارگذاری
        c.wn = wn
    except Exception:
        c.wn = None
...
def test_merge_vague_plus_head_of():
    ...
    assert tokens[0].numtype == ""
    assert tokens[0].role == "quantifier_phrase (multi-word)"
    assert tokens[0].index == 0
    assert tokens[0].original == "large numbers of"


def test_merge_with_m2_and_adv_modifiers():
    tokens = _toks(("really", "adv"), ("big", "m2"), ("piles", ""),
                   ("of", ""), ("paper", ""))
...
def test_merge_vague_labelled_as_m1_is_allowed():
    # vague quantifier ممکن است قبلاً m1 گرفته باشد — m1 «واقعی» حساب نمی‌شود
...
def test_merge_two_occurrences_in_sentence():
    tokens = _toks(("large", ""), ("numbers", ""), ("of", ""), ("cats", ""),
                   (",", ""), ("huge", ""), ("piles", ""), ("of", ""), ("dogs", ""))
    assert LawZRule().apply(tokens, _ctx()) is True
    assert _words(tokens) == ["large numbers of", "cats", ",",
                              "huge piles of", "dogs"]
...
def test_no_merge_without_modifier():
    ...
    assert _words(tokens) == ["numbers", "of", "people"]
...
def test_possessive_pronoun_in_np_blocks_rule():
    tokens = _toks(("his", "m3"), ("large", ""), ("numbers", ""),
                   ("of", ""), ("friends", ""))
    assert LawZRule().apply(tokens, _ctx()) is False
    assert _words(tokens) == ["his", "large", "numbers", "of", "friends"]


def test_possessive_s_in_np_blocks_rule():
    tokens = _toks(("John", ""), ("'s", ""), ("huge", ""), ("piles", ""),
                   ("of", ""), ("books", ""))
    assert LawZRule().apply(tokens, _ctx()) is False


def test_possessive_in_previous_np_does_not_block():
    # possessive قبل از مرز NP (ویرگول) → NP بعدی آزاد است
    tokens = _toks(("my", "m3"), ("dog", ""), (",", ""),
...
def test_real_m1_before_modifiers_blocks_merge():
    tokens = _toks(("the", "m1"), ("large", ""), ("numbers", ""),
                   ("of", ""), ("people", ""))
    assert LawZRule().apply(tokens, _ctx()) is False
    assert _words(tokens) == ["the", "large", "numbers", "of", "people"]
...
def test_no_crash_without_wordnet():
    ctx = _ctx()
    ctx.wn = None
    # بدون WordNet هنوز vague-based merge کار می‌کند
    tokens = _toks(("large", ""), ("numbers", ""), ("of", ""), ("people", ""))
    assert LawZRule().apply(tokens, ctx) is True
    assert _words(tokens)[0] == "large numbers of"
...
def test_wordnet_adjective_not_in_vague_set_is_modifier():
    ctx = _ctx()
    ctx.vague_quant_set = set()  # صفت فقط از WordNet
...
def test_verb_after_of_blocks_merge():
    # «running» با حس اول فعل نیست (اسم است) → پس از یک فعل واقعی استفاده می‌کنیم
...
def test_head_amounts_not_treated_as_np_boundary():
    # «amounts» حس فعلی دارد؛ نباید NP را قطع کند و possessive بعدش دیده نشود؟
    # اینجا possessive بعد از of است و در همان NP → قانون ممنوع
    tokens = _toks(("huge", ""), ("amounts", ""), ("of", ""), ("his", "m3"),
                   ("money", ""))
    assert LawZRule().apply(tokens, _ctx()) is False
